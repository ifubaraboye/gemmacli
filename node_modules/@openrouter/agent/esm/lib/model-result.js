...
import { hasExecuteFunction, isClientTool, isServerTool, isToolCallOutputEvent, } from './tool-types.js';
/**
    ...
    async executeToolRound(toolCalls, turnContext) {
        ...
        for (let i = 0; i < settledResults.length; i++) {
            ...
            if (value.result.error) {
                ...
                if (!isRecord(rawArgs)) {
                    throw new Error(`toolCall.arguments for "${value.toolCall.name}" must be an object after Zod validation, got ${rawArgs === null ? 'null' : Array.isArray(rawArgs) ? 'array' : typeof rawArgs}`);
                }
