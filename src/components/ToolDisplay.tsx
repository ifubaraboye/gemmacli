...
export function ToolDisplay({ events, display }: ToolDisplayProps) {
  if (display.toolDisplay === 'hidden') return null;

  if (display.toolDisplay === 'emoji') {
    return <EmojiDisplay events={events} />;
  }
  ...
  return <GroupedDisplay events={events} />;
}

function EmojiDisplay({ events }: { events: ToolEvent[] }) {
...
function GroupedDisplay({ events }: { events: ToolEvent[] }) {
  type Pending = {
    name: string;
    callId: string;
